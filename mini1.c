...
char *instruction[] = {"add", "sub", "mul", "div", "rem"};
...
//get the address of x y z
int getAddress(AST* node)
{
	
	if (node->kind == LPAR)
	{
		return getAddress(node->mid);
	}
	return (node->val - 'x') * 4;
}
...
void output_post()
{
	for (int i = 0; i < 3; i++)
	{
		if (pos[i]!=0)
		{
			printf("load r%d [%d]\n",temp, getAddress(answer[i]));
			if (pos[i] == 1)
				printf("add r%d r%d 1\n", temp, temp);
			else 
				printf("sub r%d r%d 1\n", temp, temp);
			printf("store [%d] r%d\n", getAddress(answer[i]), temp);
			temp++; 
		}
	}
}
...
void codegen(AST *root,int reg) 
{
	...
	{
	    case ASSIGN:
		    codegen(root->rhs,reg);
			flag=1;
			codegen(root->lhs,reg);
			flag=0;
			printf("store [%d] r%d\n", getAddress(root->lhs), reg);
		    return ;
	    case PREINC:
	    case PREDEC:
		    codegen(root->mid,reg);
		    if (root->kind == PREINC)
			    printf("add r%d 0 1\n",reg);
		    if (root->kind == PREDEC)
			    printf("sub r%d 0 1\n", reg);
		    printf("store [%d] r%d\n", getAddress(root->mid), reg);
		    return ;
	    case POSTINC:
	    case POSTDEC:
			temp=reg;
		    store(root);
		    codegen(root->mid,reg+1);
			return;
	    case ADD:
	    case SUB:
	    case MUL:
	    case DIV:
	    case REM:
            codegen(root->lhs,reg+1);
		    codegen(root->rhs,reg+2);
		    printf("%s r%d r%d r%d\n", instruction[root->kind - ADD], reg,reg+1,reg+2);
		    return ;
	    case MINUS:
			codegen(root->mid,reg+1); //n+1
			printf("sub r%d 0 1\n", reg);
